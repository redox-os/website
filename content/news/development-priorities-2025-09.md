...
- Redox as a Web Server Runtime, in a Virtual Machine
- Redox Server Edition
- Redox Desktop Edition
...
If you would like to help with Redox development or documentation, please join us on [Redox Chat](https://doc.redox-os.org/book/chat.html).
...
with a resume/CV or pointers to your open source work,
...
- Redox running in QEMU, with these applications
...
Redox can provide lightweight but secure containerization of web servers, databases and web service applications.
...
Redox Desktop Edition will benefit from all the features of Redox Server Edition,
but it also needs to be usable for everyone.
And Redox security must work in a natural way for the average user.

The [COSMIC Desktop](https://system76.com/cosmic/) is being developed at System 76, with Jeremy Soller as principal engineer.
...
including COSMIC Terminal, COSMIC Files, COSMIC Edit, and COSMIC Store.
...
We then plan to experiment with "containerization by default",
restricting the access of applications to only the resources that they should normally require.
We would like to create a consistent experience for containerizing applications,
...
and if we can partner with someone to build a containerized desktop,
...
In the short- and medium-term, Redox developers will be running Redox in
QEMU or VirtualBox on their Linux, Windows or MacOS laptops or desktops.

- We don't have WiFi drivers (there's a lot of code required),
so bare metal Redox is not a good choice for development right now.
- Initially, devs will need to use the GitLab remote as their trusted storage,
as work in progress might be lost if an application or the kernel panics.
...
- And, hopefully after a few months of use as a "daily driver" for devs,
...
- Rustc and Cargo reliability - due to past performance issues and a few bugs,
...
but it would be nice if we could support the official implementation.
...
For Redox Server Edition, we will have to partner with a small number of server vendors,
and develop drivers for that particular hardware.
Whether it's x86_64/AMD64, Aarch64/AMD64, or RISC-V,
we will have to find a vendor that will either provide funding or developers (or both)
to create optimized drivers for disk, network, compute acceleration,
as well as for system management and other functions.

For Redox Desktop Edition,
...
The goal for 2026 is to ensure we can run the system within its thermal envelope,
...
We would appreciate some help porting an existing written-in-Rust WiFi driver stack.
...
We are also looking for a D-Bus implementation in Rust.
